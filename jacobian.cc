...
void
Jacobian::SetMatrix (int numRows, int numCols)
{
...
void
Jacobian::SetJ1 (int numRows, int numCols)
{
...
void
Jacobian::SetJ2 (int numRows, int numCols)
{
...
void
Jacobian::SetJ3 (int numRows, int numCols)
{
...
void
Jacobian::SetJ4 (int numRows, int numCols)
{
	m_j4 = zeros(numRows, numCols);
}

void
Jacobian::CalcDPk (Graph* graph)
{
								...
								if (busK->GetType () == Bus::LOAD || busK->GetType () == Bus::LOSS_CONTROL_REACT)
									{
										...
										if (dataBranch.m_tipo == 1 && busK->GetTap () == Bus::TAP)
											{
												m_matrix (k, index) = -2 * dataBranch.m_g * (1 / tap) * vK +
																						(1 / tap) * vM *
											...
											{
												m_matrix (k, index) = -2 * dataBranch.m_g * vK +
																							(1 / tap) * vM *
																							(dataBranch.m_g * cos (theta_km) + dataBranch.m_b * sin(theta_km)) +
																							m_matrix (k, index);
											}
	...
	for (int i = 0; i < graph->GetNumBus (); i++)
		{
			...
			if (busK->GetType () == Bus::LOAD ||
						busK->GetType () == Bus::LOSS_CONTROL_REACT)
				{
					...
					for (int j = 0; j < branches.size (); j++)
						{
							...
							if (busM->GetType () == Bus::LOAD ||
										busM->GetType () == Bus::LOSS_CONTROL_REACT)
								{
									int indexM = graph->GetNumBus () - 1 + busM->GetBus ().m_ordPQ;
					...
					m_matrix (indexK, indexK) =
							(2 * busK->GetBus ().m_bsh * vK) + m_matrix (indexK, indexK);
				}
		}
}

void
Jacobian::CalcJDQ(Graph* graph)
{
	for (int i = 0; i < graph->GetNumBus (); i++)
		{
			Bus* busK = graph->GetBus (i + 1);
...
void
Jacobian::Zeros (int numRows, int numCols)
{
	m_matrix.clear ();
	m_matrix = zeros(numRows, numCols);
}
