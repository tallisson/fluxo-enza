...
void
Bus::AddBranch(Branch* branch, Bus* neighbor)
{
	...
	for (int i = 0; i < m_branches.size (); i++)
	    ...
	    if (m_bus.m_nin == dataBranch.m_nf)
	        ...
	        pgK += ( dataBranch.m_g * pow(vK, 2) - (1 / dataBranch.m_tap) *
	        			 vK * vM * ( dataBranch.m_g * cos(theta_km + dataBranch.m_def) +
	        			 dataBranch.m_b * sin(theta_km + dataBranch.m_def) ) );
	    	}
		}
...
Bus::Print(void)
{
	//std::cout << m_data.m_nin << "\t" << IOUtils::Format(m_vCalc) << IOUtils::Format(m_aCalc) << std::endl;
}
