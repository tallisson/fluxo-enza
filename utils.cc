...
Utils::Read (string filename)
{
  ...
  std::cout << ((m_s.m_baseMVA != 0), "Base MVA is equal to zero") << std::endl;
  ...
  while (true)
    {
      ...
      if (bus.m_tipo < 0 || bus.m_tipo > 3)
        {
          bus.m_tipo = 0;
          std::cout << ("Tipo da barra definido fora dos padrões CDF, considerou-se que esta barra seja do tipo (PQ)") << std::endl;
        }
      ...
      if (bus.m_v < VMIN_MIN)
        {
          bus.m_v = 1;
          std::cout << ("Magnitude de tensão definida abaixo do valor mínimo para divergência automática do caso.") << std::endl;
        }
      else if (bus.m_v > VMAX_MAX)
        {
          bus.m_v = 1;
          std::cout << ("Magnitude de tensão definida acima do valor máximo para divergência automática do caso.") << std::endl;
        }
      ...
      if (bus.m_tipo == 0)
        {
          if (bus.m_vg_o < VMIN_MIN)
            {
              bus.m_vg_o = 1;
              std::cout << ("Magnitude da tensão desejada definida abaixo do valor mínimo para divergência automática do caso.") << std::endl;
            }
          else if (bus.m_vg_o > VMAX_MAX)
            {
              bus.m_vg_o = 1;
              std::cout << ("Magnitude da tensão desejada definida acima do valor máximo para divergência automática do caso.") << std::endl;
            }
        }
      ...
      if (bus.m_tipo == 3 || bus.m_tipo == 2)
      ...
      else if(bus.m_tipo == 1)
        {
          ...
          if (bus.m_vmax == 0)
            {
              bus.m_vmax = VMAX;
              std::cout << ("Limite máximo da magnitude de tensão não definido.") << std::endl;
            }
          else if (bus.m_vmax > VMAX)
            {
              bus.m_vmax = VMAX;
              std::cout << ("Limite máximo da magnitude de tensão definido acima do valor máximo padrão") << std::endl;
            }
          else if (bus.m_vmax < VMIN)
            {
              bus.m_vmax = VMAX;
              std::cout << ("Limite máximo da magnitude de tensão definido abaixo do valor mínimo padrão") << std::endl;
            }
          // Magnitude de tensão mínima:
          ...
          if (bus.m_vmin == 0)
            {
              bus.m_vmin = VMIN;
              std::cout << ("Limite mínimo da magnitude de tensão não definido.") << std::endl;
            }
          else if (bus.m_vmin < VMIN)
            {
              bus.m_vmin = VMIN;
              std::cout << ("Limite mínimo da magnitude de tensão definido abaixo do valor mínimo padrão.") << std::endl;
            }
          else if (bus.m_vmin > VMAX)
            {
              bus.m_vmin = VMIN;
              std::cout << ("Limite mínimo da magnitude de tensão definido acima do valor máximo padrão.") << std::endl;
            }
        }
      else
        {
  ...
  if (m_s.m_posSlack == 0)
    {
      ...
      m_s.m_posSlack = 1;
      m_s.buses.at (0) = bus;
      std::cout << ("Não foi encontrada nenhuma barra do tipo SLACK. A primeira barra do tipo PV dos dados de barra foi transformada em barra SLACK") << std::endl;
    }
  ...
  while (true)
    {
      ...
      if (branch.m_tipo < 0 || branch.m_tipo > 4)
        {
          branch.m_tipo = 0;
          std::cout << ("Tipo do ramo definido fora dos padrões CDF; considerou-se que este ramo seja do tipo '0'.") << std::endl;
        }
      // Resistência série:
      ...
      std::cout << (branch.m_x != 0, "O ramo possui reatância igual a zero.") << std::endl;
      ...
      if (branch.m_t_ctrl == 0 && branch.m_tipo == 2)
        {
          ...
          std::cout << ("Barra controlada pelo trafo do ramo não definida.") << std::endl;
        }
      ...
      if (branch.m_tipo == 0)
        ...
        {
          branch.m_tap = 1.0;
          std::cout << ("Tap do trafo do ramo não definido.");
        }
      ...
      if (branch.m_tipo == 2 || branch.m_tipo == 3)
        {
          ...
          if (branch.m_tapmin == 0)
            {
              ...
              std::cout << ("Limite mínimo do tap do trafo não definido.") << std::endl;
            }
          else if (branch.m_tapmin < TAPMIN_MIN || branch.m_tapmin > 1)
            {
              branch.m_tapmin = TAPMIN_MIN;
              std::cout << ("Limite mínimo do tap do trafo do ramo definido fora dos valores operacionais.") << std::endl;
            }
          // Tap máximo:
          ...
          if (branch.m_tapmax == 0)
            {
              ...
              std::cout << ("Limite máximo do tap do trafo não definido.") << std::endl;
            }
          else if (branch.m_tapmax > TAPMAX_MAX || branch.m_tapmax < 1)
            {
              branch.m_tapmax = TAPMAX;
              std::cout << ("Limite máximo do tap do trafo definido fora dos valores operacionais.") << std::endl;
            }
          ...
          if (branch.m_passo == 0)
            {
              branch.m_passo = STEP;
              std::cout << ("Tamanho do passo entre posições intermediárias do tap do trafo não definido.") << std::endl;
            }
